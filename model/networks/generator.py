...
class PoseGenerator(BaseNetwork):
    def __init__(self, image_nc=3, structure_nc=18, output_nc=3, ngf=64, norm='instance', 
        ...
        self.Zencoder = Zencoder(3, ngf) #encoder that gets image S as input
        # (combination of  encoder and per region encoding)
        ...
        self.parenc = HardEncoder(8+18+8+3, ngf) #encoder that gets parsing and 3 other inputs

        self.dec = BasicDecoder(3)

        self.efb = EFB(ngf*4, 256)#spatial aware normalization $ per region normalization
    ...
    def forward(self, img1, img2, pose1, pose2, par1, par2):
        codes_vector, exist_vector, img1code = self.Zencoder(img1, par1) # Fi
       ...
       # print(SPL2_img.shape,SPL1_img.shape)
        ...
        par2 = SPL2_onehot
        '''
        parcode,mask = self.parnet(torch.cat((par1, pose1, pose2),1)) #parsing utput
        par2 = parcode
        
        parcode = self.parenc(torch.cat((par1,par2,pose2, img1), 1)) #Fp
        ...
        img2code = self.imgenc(img2)#VGG output of original image
        ...
        if True:
            ...
            print("----")
            print(img1code.shape, parcode.shape)
            parcode1 = feature_normalize(parcodze)
